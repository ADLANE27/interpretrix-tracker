      ...
      <CardContent className="p-2 relative flex flex-col h-full justify-between">
        <div className="mb-2 flex items-center gap-2">
          <div className="flex items-center gap-2 w-full">
            <h3 className="text-xl font-bold text-gradient-primary leading-tight truncate">
              {lastName}
            </h3>
            {firstName && (
              <h3 className="text-xl font-bold text-gradient-primary leading-tight truncate">
        ...
        <div className="flex flex-wrap gap-1 mb-2 items-center">
          ...
          <Badge variant="outline" className={`text-[13px] px-2 py-1 flex items-center gap-1 ${locationConfig[workLocation].color}`}>
            <LocationIcon className="h-4 w-4" />
            <span>{workLocation === "remote" ? "Télétravail" : "Sur site"}</span>
          </Badge>
          
          <Badge variant="outline" className="text-[13px] px-2 py-1 bg-gray-50">
        ...
        {showAnyTarif && (
          <div className="flex flex-wrap gap-1 mb-2">
            {showTarif5min && (
              <Badge variant="outline" className="text-[13px] bg-gray-50">
          ...
          <div className="grid grid-cols-2 gap-x-1 gap-y-0.5 text-sm text-foreground mb-2">
            {interpreter.booth_number && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-[13px]">Cabine {interpreter.booth_number}</span>
              </div>
            )}
            {interpreter.phone_number && (
              <div className="flex items-center gap-1">
                <Phone className="h-4 w-4 text-palette-ocean-blue" />
                <span className="text-[13px]">{interpreter.phone_number}</span>
              </div>
            )}
            {interpreter.landline_phone && (
              <div className="flex items-center gap-1">
                <PhoneCall className="h-4 w-4 text-palette-ocean-blue" />
                <span className="text-[13px]">{interpreter.landline_phone}</span>
              </div>
            )}
            {interpreter.private_phone && (
              <div className="flex items-center gap-1">
                <Phone className="h-4 w-4 text-palette-ocean-blue" />
                <span className="text-[13px]">{interpreter.private_phone}</span>
              </div>
            )}
            {interpreter.professional_phone && (
              <div className="flex items-center gap-1">
                <Phone className="h-4 w-4 text-palette-ocean-blue" />
                <span className="text-[13px]">{interpreter.professional_phone}</span>
              </div>
            )}
        ...
        {hasFutureMission && interpreter.next_mission_start && (
          <div className="mb-1">
            <UpcomingMissionBadge
              ...
              className="bg-red-500 text-white text-[13px]"
            />
          </div>
        )}
