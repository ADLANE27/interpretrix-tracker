        ...
        <CardContent className="p-2 relative flex flex-col h-full justify-between">
          <div className="mb-2 flex items-center gap-2">
            <div className="flex items-center gap-2 w-full">
              <h3 className="text-sm font-bold text-gradient-primary leading-tight truncate">
                {lastName}
              </h3>
              {firstName && (
                <h3 className="text-sm text-gradient-primary leading-tight truncate">
                  {firstName}
                </h3>
              )}
            </div>
          ...
          <div className="flex flex-wrap gap-1 mb-2 items-center">
            ...
            <Badge variant="outline" className={`text-[9px] px-1 py-0.5 flex items-center gap-0.5 ${workLocationConfig[workLocation].color}`}>
              <LocationIcon className="h-2 w-2" />
              <span>{workLocationLabels[workLocation]}</span>
            </Badge>
            
            <Badge variant="outline" className="text-[9px] px-1 py-0.5 bg-gray-50">
          ...
          {hasAnyPhoneNumber && (
            <div className="grid grid-cols-2 gap-x-1 gap-y-0.5 text-[10px] text-foreground mb-2">
              {interpreter.booth_number && (
                <div className="flex items-center gap-1">
                  <User className="h-2.5 w-2.5 text-palette-ocean-blue" />
                  <span className="text-[9px]">Cabine {interpreter.booth_number}</span>
                </div>
              )}
              {interpreter.phone_number && (
                <div className="flex items-center gap-1">
                  <Phone className="h-2.5 w-2.5 text-palette-ocean-blue" />
                  <span className="text-[9px]">{interpreter.phone_number}</span>
                </div>
              )}
              {interpreter.landline_phone && (
                <div className="flex items-center gap-1">
                  <PhoneCall className="h-2.5 w-2.5 text-palette-ocean-blue" />
                  <span className="text-[9px]">{interpreter.landline_phone}</span>
                </div>
              )}
              {interpreter.private_phone && (
                <div className="flex items-center gap-1">
                  <Phone className="h-2.5 w-2.5 text-palette-ocean-blue" />
                  <span className="text-[9px]">{interpreter.private_phone}</span>
                </div>
              )}
              {interpreter.professional_phone && (
                <div className="flex items-center gap-1">
                  <Phone className="h-2.5 w-2.5 text-palette-ocean-blue" />
                  <span className="text-[9px]">{interpreter.professional_phone}</span>
                </div>
              )}
